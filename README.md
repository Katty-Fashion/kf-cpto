...
- **Knowledge Bundle (OKF)** — An agent-consumable [Open Knowledge Format](https://github.com/GoogleCloudPlatform/knowledge-catalog/tree/main/okf) bundle at `docs/okf/`, generated alongside the dashboard so our own tooling and agents can read the whole picture without bespoke parsing (see [Knowledge Bundle (OKF)](#knowledge-bundle-okf--agent-consumable-project-memory))
...
## Knowledge Bundle (OKF) — Agent-Consumable Project Memory

Alongside the human dashboard, we emit an **[Open Knowledge Format](https://github.com/GoogleCloudPlatform/knowledge-catalog/tree/main/okf) (OKF v0.1)** bundle at [`docs/okf/`](docs/okf/) — a directory of cross-linked markdown + YAML-frontmatter "concept" files that any agent (or human) can read without bespoke parsing. It is generated by [`scripts/okf_export.py`](scripts/okf_export.py) as a **pure transform** of the data we have already parsed — it never re-reads `kanban.md` (the one-parser rule).

### What's in the bundle

| Concept | `type` | Built from |
| :--- | :--- | :--- |
| One file per tracked project | `Project` | parsed kanban + `loe.yml` rollup + `depends_on` cross-links + a delivery line read from each repo's GSD `.planning/STATE.md` |
| LOE and RAG-status definitions | `Metric` | canonical definitions, including the person-days vs working-day-span distinction |
| Migration milestones | `Milestone` | `docs/_data/calendar.yml` |

Plus OKF's reserved `index.md` (progressive-disclosure navigation) and `log.md` (change history). The bundle is Jekyll-excluded, so it ships as raw portable markdown, and it is **deterministic** — re-running the aggregator produces a byte-identical bundle (no run-timestamps in the bodies).

### Why we do this — the process value

- **[REUSE] One substrate for our agents.** The Activity Sync skill (and our Claude sessions) used to re-assemble context from scattered `kanban.md`, GSD `.planning/`, and the migration plan on every run. The OKF bundle is that context in one cross-linked place — exactly the "stop solving context-assembly from scratch" problem OKF was built for.
- **[SINGLE-SOURCE] The LOE metric is defined once.** Our recurring "is effort person-days or calendar span?" ambiguity now lives in a single `Metric` concept at [`docs/okf/metrics/loe.md`](docs/okf/metrics/loe.md) instead of tribal knowledge.
- **[BRIDGE] Board and delivery reality are linked as data.** Each project concept ties its migration tasks to its real GSD delivery state (milestone + percent), so the drift we reconcile every week is visible as machine-readable data, not just a manual judgment call.
- **[PORTABLE] No new dependency, no lock-in.** OKF is just markdown + frontmatter — we adopted the *format*, not a platform. There is **no Google Cloud or Knowledge Catalog dependency**; the bundle is readable by our own tools, the OKF reference visualizer, or any future consumer.

### What it is not

OKF is a **representation** layer, not a sync engine — it makes our knowledge legible and shareable, but reconciliation still lives in [`reconcile.py`](.claude/skills/activity-sync/reconcile.py) and [`generate_kanban.py`](scripts/generate_kanban.py). Conformance is enforced by [`scripts/validate_okf.py`](scripts/validate_okf.py), which runs **non-blocking** in CI: a malformed bundle is surfaced in the logs but is never allowed to block the canonical dashboard.
