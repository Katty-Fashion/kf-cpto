...
kanban
  Todo
    ...
    task28["R3-AAS: Low"]
    task32["kf-be-platform: IDP setup (Keycloak/Auth0/logTo) + SMTP server"]
    task33["kf-be-platform: RBAC system (scopes, claims, middleware)"]
    task34["kf-be-platform: Tenant management (CRUD, provisioning, S3 prefix)"]
    task35["kf-be-platform: EPCIS Export (JSON, PDF, GS1 standard, possibly signed)"]
    task36["kf-be-platform: LLM Ecodesign full integration — WP4 T4.1"]
    task37["kf-be-platform: IoT Adapter & Event Log (MQTT) — T2.5 ALADIN"]
    task38["kf-be-platform: Notifications multi-channel (email, SMS, webhook, in-app)"]
    task39["kf-be-platform: Data migration scripts KF → ALADIN (one-shot + rollback)"]
    task41["kf-fe-platform: Login flow + redirect handling"]
    task42["kf-fe-platform: Admin Console UI (Platform Admin operations)"]
    task43["kf-fe-platform: Overview refactor (dynamic widgets per rol)"]
    task44["kf-fe-platform: Models Page refactor (filtere per rol, search)"]
    task45["kf-fe-platform: Tech Pack layout (sidebar, tooltips, guide)"]
    task46["kf-fe-platform: Model Sheet fixes (imagini, reconciliere BOM)"]
  ...
  In-Progress
    ...
    task26["R3-AAS: High"]
    task29["R3-AAS: ---"]
    task30["kf-be-platform: Database schema v2 design + migrations (multi-tenant RLS)"]
